...
class ActiveParty:
    def __init__(self,passive_depth = 2,active_depth=3) -> None:
        ...
        self.passive_depth = passive_depth  #分层书被动方层数
        self.active_depth = active_depth  #分层书主动方层数
    ...
    def sample_align(self):
        ...
        # 被动方返回哈希后的样本列表,求交集
        def recv_sample_list() -> bool:
            nonlocal train_hash, valid_hash

            for pp in pp_list:
                recv_data=pp.get_sample_align() #文件地址，传递了索引的hash  : hash_data['train_hash']
    ...
    def train(self):
        ...
        while self.train_status(): # 树没建完
            ...
            if depth < self.passive_depth:
                # 广播梯度
                def send_gradients(instance_space_file: str):
                ...
                def get_splits_sum() -> bool:
                    ...
                    for pp in pp_list:
                        ...
                        passive_best_split = self.passive_best_split_score(recv_data['file_name'], full_grad_sum, full_hess_sum)  # (idx, split_score)
                        global_splits = (self.reverse_passive_port[pp], passive_best_split[0], passive_best_split[1])
                    return True
                get_splits_sum()
            ...
            if depth >= self.passive_depth or global_splits[2] < 0:
                # 主动方计算最佳分裂点
            ...
            if global_splits[0] == self.name:           # 最佳分裂点属于主动方
                ...
                pp = self.passive_port[global_splits[0]]      # 找到对应的被动方
                data = msg_split_confirm(global_splits[0], global_splits[1])
                recv_data=pp.confirm_split(data) #被动方将最佳分裂
...
class TreeNode:
    def __init__(self, id: int=0, instance_space: list=None,depth=0) -> None:
        self.id = id                            # 节点标号
    ...
    def split(self, party_name, left_space, feature_split=None, look_up_id=0):
        ...
        self.left, self.right = TreeNode(self.id * 2 + 1, left_space ,depth=self.depth+1), TreeNode(self.id * 2 + 2, right_space,depth=self.depth+1)
